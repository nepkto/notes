# AWS Deployment Guide: .NET 8 with SQL Server on EC2 or Docker

This guide shows two supported ways to host a .NET 8 web application with SQL Server on an Ubuntu server in AWS:

1. Native Ubuntu deployment with `systemd`, `nginx`, and `certbot` on an EC2 instance.
2. Containerized deployment with Docker Compose, `nginx`, `certbot`, and a SQL Server container.

Use the native path if you want the simplest operational model on a single EC2 instance. Use Docker if you want clearer isolation and easier local parity with production.
...
- [Architecture at a Glance](#architecture-at-a-glance)
  ...
  - [Native EC2 path](#native-ec2-path)
  - [Docker path](#docker-path)
- [AWS Setup](#aws-setup)
- [Common Prerequisites](#common-prerequisites)
- [Option 1: Native Ubuntu EC2 Deployment](#option-1-native-ubuntu-ec2-deployment)
  - [1. Install .NET 8 on the instance](#1-install-net-8-on-the-instance)
...
- How to provision an AWS Ubuntu EC2 instance safely.
- How to expose only the required network ports using a security group.
...
- Compute: Amazon EC2 running Ubuntu 22.04 LTS or another supported Ubuntu LTS release.
- Web app: .NET 8.
...
- In the native EC2 path, Kestrel runs as a `systemd` service on the instance.
...
```mermaid
flowchart TB
    user[User / Browser] --> sg[AWS Security Group: 80, 443, 22]

    subgraph native[Native Ubuntu EC2]
...
- Public traffic hits the AWS security group on ports 80 and 443.
- `nginx` terminates TLS and proxies to Kestrel on `localhost:5000`.
- The app runs as a `systemd` service.
- SQL Server runs directly on the EC2 instance.
...
3. Attach a security group that allows only these inbound ports:
...
Install tools you will need on the instance:
...
## Option 1: Native Ubuntu EC2 Deployment

### 1. Install .NET 8 on the instance
...
Install `dotnet-sdk-8.0` as well if you plan to build on the instance.
...
Copy the published files to the instance:

```bash
scp -r ./publish/* ubuntu@YOUR_EC2_PUBLIC_IP:/var/www/dotnetapp/
```
...
- If Let's Encrypt issuance fails, verify DNS points to the EC2 public IP and that port 80 is reachable.
