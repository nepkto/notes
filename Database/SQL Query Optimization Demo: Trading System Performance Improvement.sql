...
## The Real-World Discussion That Led to This Solution

**Background:** FarmTech Energy Solutions is experiencing performance issues with their commodity trading reporting system. The main P&L reporting query is taking 45+ seconds to execute, causing timeouts and frustrated traders during market hours.

### **Mike Rodriguez** - *Lead Trading Systems Analyst*  
### **Sarah Chen** - *Senior Database Developer*

---

#### **Mike:** "Sarah, we've got a major problem..."

**Mike:** The morning P&L report is killing us again. Traders are screaming because they can't get their risk positions during market open. The query is timing out after 45 seconds, and when it does run, it's using 80% of our server CPU.

**Sarah:** Let me take a look at this query. *[Opens the trading system query]* Oh wow, this is a complex beast - 200+ lines with multiple OUTER APPLY operations. I can see several red flags right away.

---

#### **Sarah:** "I see the main performance killers..."

**Sarah:** Mike, look at these OUTER APPLY blocks:
...
OUTER APPLY (
    ...
    WHERE id IN (
        ...
        WHERE effective_date <= '2025-05-01'
            AND debt_rating = cci.Risk_rating  -- This runs for EVERY deal!
    )
) dp
```

**Mike:** What's wrong with that? We need the latest default probability for each counterparty's credit rating.

**Sarah:** The problem is **performance**. Each OUTER APPLY executes a correlated subquery for every single deal row. If you have 1,000 deals, this subquery runs 1,000 times! Plus, you have multiple OUTER APPLYs doing the same thing.

**Mike:** So instead of getting the latest rates once per credit rating, we're getting them once per deal?

**Sarah:** Exactly! It's like going to the grocery store separately for each item instead of making one trip with a shopping list.

---

#### **Mike:** "Show me the numbers..."

**Mike:** How bad is it really? Give me some concrete numbers.

**Sarah:** Let's break it down:

| Current Approach (OUTER APPLY) | Optimized Approach (CTE) |
|-------------------------------|-------------------------|
| 1,000 deals × 6 OUTER APPLYs = 6,000 subqueries | 6 one-time calculations |
| Each subquery scans entire tables | Single table scan with window functions |
| ~50,000 total table operations | ~1,006 total operations |
| **45+ seconds execution time** | **Expected: 2-3 seconds** |

**Mike:** That's a 99% reduction in operations! But how do we fix it without breaking the business logic?

---

#### **Sarah:** "Here's my optimization plan..."

**Sarah:** I'll show you three approaches. Let's start with the CTE method since it's the cleanest:

```sql
-- BEFORE: Multiple OUTER APPLY operations (current nightmare)
OUTER APPLY (
    SELECT probability FROM default_probability 
    WHERE id IN (SELECT MAX(id)...)  -- Runs 1000 times!
) dp

-- AFTER: Pre-calculate once with CTE
...
-- Then simple JOIN in main query
LEFT JOIN LatestCreditRates lcr ON lcr.debt_rating = cci.Risk_rating AND lcr.rn = 1
```

**Mike:** I like this! But what about the FV (Fair Value) calculations? Those OUTER APPLYs look even more complex.

---

#### **Sarah:** "The FV mappings are the worst offenders..."

**Sarah:** You're right! Look at this monstrosity:
...
OUTER APPLY (
    SELECT fv_level_value_id, MAX(effective_date) effective_date
    FROM fv_report_group_deal  
    WHERE source_deal_header_id = sdpd.source_deal_header_id
        AND effective_date = fv_d_mx.effective_date  -- Depends on previous OUTER APPLY!
    GROUP BY fv_level_value_id
) frgd
```

**Mike:** So this is doing two dependent lookups per row? No wonder it's slow!

**Sarah:** Exactly! And we can replace all of this with a single window function:

```sql
WITH LatestFVMappings AS (
    SELECT 
        source_deal_header_id,
        term_start,
        fv_level_value_id,
        ROW_NUMBER() OVER (
            PARTITION BY source_deal_header_id, term_start 
            ORDER BY effective_date DESC
        ) as rn
    FROM fv_report_group_deal
    WHERE effective_date <= '2025-05-01'  -- Your PnL date
)
-- Simple JOIN instead of nested OUTER APPLYs
LEFT JOIN LatestFVMappings lfv ON lfv.source_deal_header_id = sdpd.source_deal_header_id 
    AND lfv.term_start = sdpd.term_start 
    AND lfv.rn = 1
```

---

#### **Mike:** "What about indexing strategy?"

**Mike:** This looks great, but we need the right indexes to support this, right?

**Sarah:** Absolutely! Here are the critical indexes we need:

```sql
-- For the main PnL join (most critical)
CREATE NONCLUSTERED INDEX IX_source_deal_pnl_detail_optimized
...
-- For credit risk lookups
CREATE NONCLUSTERED INDEX IX_counterparty_credit_info_lookup
ON counterparty_credit_info (counterparty_id) 
INCLUDE (Risk_rating);
```

**Mike:** How much improvement are we talking about realistically?

**Sarah:** Based on similar optimizations I've done:
- **Current**: 45+ seconds, high CPU usage
- **Expected after optimization**: 2-5 seconds, minimal CPU impact
- **Memory usage**: Reduced by ~70% due to eliminating correlated subqueries
- **Concurrent user support**: Should handle 10x more simultaneous reports

---

#### **Final Outcome**

**Mike:** "Sarah, this is exactly what we needed. The traders will be thrilled to get their reports in under 5 seconds instead of waiting 45+ seconds or hitting timeouts."

**Sarah:** "The best part is this approach is reusable. We can apply the same OUTER APPLY to CTE/JOIN conversion pattern to other slow queries in the system. This single optimization will probably improve overall system performance by 30-40%."

**Mike:** "Let's get started on those indexes today. I'll coordinate the maintenance window with the trading desk."
